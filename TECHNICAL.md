...
| `prefix.<mode>=<char>` | The prefix character for a given TUI mode. The full set: `prefix.notes` (default `@`), `prefix.todo` (`!`), `prefix.directories` (`#`), `prefix.panes` (`*`), `prefix.jira` (`-`), `prefix.files` (`~`), `prefix.tags` (`$`), `prefix.codegraph` (`&`), `prefix.ag` (`,`), `prefix.paperless` (`<`). All other ASCII prefix chars are taken. | mode's default |
...
2. **Mode-specific snapshot priming**: directories mode
   ...
   ag / files / segments / similar modes are no-ops here (their
