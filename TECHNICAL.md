...
2. **Mode-specific snapshot priming**: directories mode
   ...
   ag / files / elements modes are no-ops here (their
...
| Elements      | `:`    | Search individual paragraphs, list items, and headings via `note_search`'s `elements` table — finer-grained than `@` (Notes), which searches whole files. The search is debounced (400ms, `ELEMENTS_DEBOUNCE`) and runs on a background thread — same `*State`/`*Request`/mpsc-channel architecture as `,` (ag) mode's `AgState`, via `App::elements_touch()` / `elements_maybe_autocall()` / `process_elements_result()` — so typing stays responsive even for an unbounded `:` (empty) query; `Esc` cancels an in-flight search. Selecting a row stages `$EDITOR <file> +<start_line>` (same convention as `$` / `,` / `&`). Full Obsidian-like query DSL (`#tag`, `[[link]]`, `[attr:value]`, `(a OR b)`, bare-word AND) via `note_search::parse_query` → `criteria.query_expr`, same mechanism `@` / `!` mode use — tags/links cascade from headings and frontmatter to every element in scope. `Tab` completion works too (same `notes.database` tag/link namespace as `@` / `!`). The details pane shows a 50-line RAW (unannotated) window of the file CENTERED on the selected row's own `start_line`, piped through `bat` — the same "clean markdown in, highlighted markdown out" pipeline `@` / `!` mode use (deliberately NOT `$` / `,` mode's `read_source_context_with_cache`, which prefixes lines with numbers and a `>>` match marker that isn't valid markdown and would fight `bat`'s highlighting) — not just that element's own isolated text; for a file shorter than the window this covers the entire file, for a longer one the match stays visible without scrolling from the top; the highlighted result is cached per (file, line) in `ElementsState::context_cache` so re-selecting the same row on a later keystroke is a map lookup, not another `bat` spawn. `build_merged_rows` takes the same cheap early-return path as panes mode for elements rows (`self.rows.clone()`, no labeled-row interleave/re-sort) rather than the command-history "labeled rows" merge — that merge runs unconditionally on every keystroke and would otherwise dominate the cost on a large notes vault regardless of how fast the backgrounded search itself is. `draw_list` (`src/tui/render.rs`) also only builds a `ListItem` per row actually visible in the viewport (`list_visible_window`) rather than for every row in `merged_rows` — the real dominant cost reported as elements-mode typing lag turned out to be here, not the search: with tens of thousands of indexed elements, building styled `ListItem`s for the FULL result set on every keystroke's redraw (ratatui's `List` widget only ever *paints* the visible slice, but the construction cost for the rest was already paid) measured at ~400ms/keystroke, independent of every other fix in this section. Needs `notes.database` + `notes.dir`, and a `note_search` index built with element-search + query support (`smarthistory check --prefix :` reports a clear error otherwise). See `docs/modes/elements.md`. |
