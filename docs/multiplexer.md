...
A `# hosts` block at the bottom of the panes view. Each `host.<key>` from `~/.config/smarthistory/hosts` (or the main config file — see [`docs/configuration.md#hostkey`](configuration.md#hostkey)), merged with `~/.ssh/config`, becomes a row; selecting a row either focuses an existing workspace already running that host's `ssh` connection, or creates a new workspace and bootstraps the `ssh` body into its first pane.
...
The `session.<key>.exec` config key (or a per-host equivalent) is the command to bootstrap a project after focus / create. The TUI runs it via `send_in_pane_command`:
