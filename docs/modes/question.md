...
Question mode sends the body of the query to the configured ollama instance and
shows the model's 4-sentence answer in an overlay. Useful for short factual
questions where you don't want a Bash command — you want a text reply you can
read.
...
not a list. The `Enter` action stages the question for re-execution rather than
selecting a row.
...
Same as LLM command mode: 1 second after the last keystroke. `Ctrl-C` / `Esc`
cancels the request without leaving the TUI.
