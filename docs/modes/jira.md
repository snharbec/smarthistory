...
- `Ctrl-]` (Smart open) opens the marked issues' URLs in the browser **in the background** (or just the selected issue when nothing is marked) — spawned as detached child processes so the TUI stays open. One opener process per issue (`xdg-open` on Linux only reliably opens one URL per invocation). The status message names every key opened. The default key is `C-]`, an ASCII control char every terminal emits reliably.
...
The same `Tab` binding (`Action::JiraFieldComplete`) is also the tag / link completion key in [`@` (Notes) mode](notes.md), [`!` (Todo) mode](todo.md), [`:` (Segments) mode](segments.md), [`"` (Similar) mode](similar.md) — all four share the same `notes.database` tag/link namespace — and the tag / correspondent completion key in [`<` (Paperless) mode](paperless.md). The dispatch site checks the active mode and routes to the right completion function:
