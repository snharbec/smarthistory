...
The same `Tab` binding (`Action::JiraFieldComplete`) is also the tag / link completion key in [`@` (Notes) mode](notes.md), [`!` (Todo) mode](todo.md), and [`:` (Segments) mode](segments.md) — all three share the same `notes.database` tag/link namespace. The dispatch site checks the active mode and routes to the right completion function:

| Active mode | Tab dispatches to | Completes |
| --- | --- | --- |
| `-` (JIRA) | `jira_field_complete_at_cursor` | JQL field names (and `@`-prefixed aliases / fragments) |
| `@` (Notes), `!` (Todo), and `:` (Segments) | `notes_tab_complete_at_cursor` | Note tags (after `#`) and link names (after `@`) |
