...
| `^` (Browser) | stages `note_search convert <url>` for the **selected** row, converting the bookmark/history entry into a local markdown note instead of opening its URL (which is what plain `Enter` still does); single-row only, marks are not consulted |
