...
mod tests {
    ...
    fn session_row_in_panes_mode_stages_create_with_exec() {
        ...
        let mut app = directories_test_app(&[]);
        app.multiplexer =
            crate::multiplexer::backend_for(crate::multiplexer::MultiplexerKind::Herdr);
        app.query = "*".to_string();
        app.refresh();
    ...
    fn setup_todo_db() -> (std::path::PathBuf, std::path::PathBuf) {
        ...
        for entry in fs::read_dir(&dir).expect("read dir") {
            ...
            let data = note_search::markdown_parser::process_markdown_file(&path, &dir)
                .expect("process file");
    ...
    fn fetch_todos_orders_lines_within_a_file() {
        ...
        note_search::init_database_schema(&conn)
            ...
            .expect("init schema");
        let data =
            note_search::markdown_parser::process_markdown_file(&dir.join("single.md"), &dir)
                .expect("process file");
        note_search::write_markdown_data_to_sqlite_with_conn(&data, &conn)
            .map_err(|e| format!("write: {e}"))
            .expect("write db");
    ...
    fn fetch_directories_layout_swap() {
        ...
        let mut app =
            directories_test_app(&[("ls -la /tmp/foo bar", "/Users/har/work/project", 60)]);
        app.query = "#".to_string();
        app.refresh();
    ...
    fn parse_tmux_pane_line_real_world_output() {
        ...
        let windows: Vec<TmuxWindowInfo> =
            sample.lines().filter_map(parse_tmux_pane_line).collect();
        assert_eq!(
            windows.len(),
            3,
    ...
    fn select_t_marked_directory_stages_select_and_switch() {
        ...
        let staged = app.selection.as_deref().expect("selection must be set");
        assert!(
            staged.contains("tmux select-pane -t %2")
                && staged.contains("tmux switch-client -t %2"),
    ...
    fn select_unmarked_directory_stages_new_session_and_switch() {
        ...
        assert!(
            staged.contains("tmux new-session -d -s coolthing -c ~/Projects/coolthing")
                || staged
                    .contains("tmux new-session -d -s coolthing -c /Users/har/Projects/coolthing"),
            "staged command must create detached session with the directory basename, got: {staged:?}"
        );
        assert!(
