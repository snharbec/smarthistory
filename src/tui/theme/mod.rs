...
/// The active color scheme, light or dark. Used by the
/// `theme.light=` / `theme.dark=` config keys (which let
/// the user pick a separate built-in theme for each
/// scheme) and persisted across TUI invocations in the
/// session file's `colorscheme=` line so
/// `Action::ToggleColorScheme` sticks between launches.
///
...
/// (the loader then picks it up), toggle to `Light` with
/// `Action::ToggleColorScheme`, or pick a theme from the
/// in-TUI picker, which writes the active scheme's slot
/// on commit.
#[derive(Debug, Default, PartialEq, Eq, Clone, Copy, Hash)]
...
impl ColorScheme {
    ...
    /// file (`theme.light=...` / `theme.dark=...`), the
    /// session file's `colorscheme=` line, and in status
    /// messages / theme-picker hints.
    ...
    /// Parse a session-file or config-file scheme value
    /// (case-insensitive `"light"` / `"dark"`). Returns
    /// `None` for anything else so a hand-edited file
    /// with a garbled value can't wedge the TUI on
    /// startup.
...
mod scheme_tests {
    ...
    /// (`theme.light` / `theme.dark`), the session
    /// file's `colorscheme=` line, status messages,
    /// and the theme picker. This is the single
    ...
    /// `ColorScheme::parse()` round-trips `label()`
    /// and is case-insensitive; anything else is
    /// rejected so a hand-edited session/config file
    /// can't wedge the TUI on startup.
    #[test]
    ...
    fn parse_rejects_garbage() {
        ...
        assert_eq!(ColorScheme::parse("blue"), None);
    }
}
